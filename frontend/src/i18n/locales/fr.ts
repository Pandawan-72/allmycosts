const COMMON = {
  app: { name: "All My Costs", tagline: "Suivez vos abonnements en un coup d'œil." },
  common: {
    cancel: "Annuler", save: "Enregistrer", add: "Ajouter", delete: "Supprimer",
    confirm: "Confirmer", close: "Fermer", continue: "Continuer", back: "Retour",
    yes: "Oui", no: "Non", ok: "OK", retry: "Réessayer",
    monthly: "Mensuel", yearly: "Annuel", lifetime: "À vie", optional: "(optionnel)",
    none: "Aucun", loading: "Chargement…",
  },
  auth: {
    welcome: "Bienvenue.", welcomeName: "Bienvenue, {{name}} 👋", greeting: "Bonjour, {{name}} 👋",
    createAccount: "Créer un compte.",
    subtitlePro: "Maîtrisez vos dépenses récurrentes.",
    email: "E-mail", password: "Mot de passe", name: "Nom",
    signIn: "Se connecter", signUp: "Créer mon compte",
    signingIn: "Connexion…", signingUp: "Création…",
    or: "ou", continueWithGoogle: "Continuer avec Google",
    noAccount: "Pas encore de compte ?", goSignUp: "Créer un compte",
    haveAccount: "Déjà un compte ?", goSignIn: "Se connecter",
    pwdMin: "Le mot de passe doit contenir au moins 6 caractères.",
    fillAll: "Tous les champs sont requis.", fillCreds: "Veuillez renseigner vos identifiants.",
    invalidCreds: "Identifiants invalides.", emailExists: "Un compte avec cet e-mail existe déjà.",
    signInFailed: "Échec de la connexion.", signUpFailed: "Échec de la création du compte.",
    googleFailed: "Échec Google.", emailPh: "vous@exemple.com", passwordPh: "••••••••",
  },
  home: {
    totalMonthly: "TOTAL MENSUEL", totalYearly: "TOTAL ANNUEL",
    subsCount_one: "{{count}} abonnement en {{currency}}",
    subsCount_other: "{{count}} abonnements en {{currency}}",
    yourSubs: "Vos abonnements",
    empty: "Aucun abonnement pour l'instant. Touchez le bouton + pour en ajouter un.",
    trialBanner: "Essai gratuit — {{hours}}h restantes",
    trialEnded: "Essai gratuit terminé", upgrade: "Passer Pro",
    nextPayment: "Prochain paiement", noSubs: "Aucun abonnement",
    addFirst: "Ajoutez d'abord des abonnements pour exporter un PDF.",
    pdfError: "Impossible de générer le PDF.",
  },
  sub: {
    newSub: "Nouvel abonnement", editSub: "Modifier",
    name: "Nom", namePh: "Netflix, Spotify, etc.",
    price: "Prix", cycle: "Cycle", category: "Catégorie",
    dueDate: "Date de prochain paiement", pickDate: "Choisir une date",
    clearDate: "Effacer", newCategory: "Nouvelle", customCat: "Nouvelle catégorie",
    customName: "Nom", customNamePh: "Ex : VPN, Coaching…",
    icon: "Icône", color: "Couleur", create: "Créer",
    nameRequired: "Nom requis.", priceInvalid: "Prix invalide.",
    dateInvalid: "Date invalide. Format YYYY-MM-DD attendu.",
    pickCurrency: "Choisir une devise", dateOptional: "Date de prochain paiement (optionnel)",
    deleteConfirm: "Supprimer \"{{name}}\" ?",
  },
  settings: {
    title: "Paramètres", preferences: "Préférences", account: "Compte",
    baseCurrency: "Devise principale", language: "Langue",
    proPlan: "Abonnement Pro", manage: "Gérer",
    notifications: "Rappels de paiement", logout: "Se déconnecter",
    logoutConfirm: "Voulez-vous vraiment vous déconnecter ?",
    notifEnabled: "Activés", notifDisabled: "Désactivés",
    notifDays: "Rappel {{days}} jour(s) avant",
  },
  paywall: {
    title: "Passez Pro.", subtitle: "Maîtrisez tous vos abonnements sans limite.",
    feat1: "Abonnements illimités",
    feat2: "Export PDF + partage e-mail",
    feat3: "Rappels d'échéance",
    feat4: "Conversion FX automatique",
    feat5: "Statistiques par catégorie",
    feat6: "Catégories personnalisées illimitées",
    popular: "LE PLUS POPULAIRE", save: "Économisez 33 %", trial: "Essai 48 h",
    perMonth: "/ mois", perYear: "/ an", once: "paiement unique",
    monthlyDesc: "Facturé chaque mois", yearlyDesc: "Soit {{price}}/an, facturé une fois",
    lifetimeDesc: "Payez une fois, accédez à vie",
    chooseMonthly: "Choisir Mensuel", chooseYearly: "Choisir Annuel", chooseLifetime: "Acheter à vie",
    trialEnded: "Votre essai gratuit de 48h est terminé. Choisissez une offre pour continuer.",
    inTrial: "Essai gratuit en cours — encore {{hours}}h", restoreNote: "Paiement sécurisé via l'App Store / Google Play.",
    restore: "Restaurer mes achats",
    storeNote: "Le paiement est traité par l'App Store (iOS) ou Google Play (Android).",
    webNote: "Sur cette version web, l'achat est simulé. Téléchargez l'app mobile pour souscrire réellement.",
    successTitle: "Bienvenue dans Pro !", successDesc: "Toutes les fonctionnalités sont débloquées.",
    inFxNote: "Prix affichés convertis depuis l'EUR via le taux du jour. Facturation effective en EUR.",
    inLocal: "Soit environ {{amount}}",
    finalizingTitle: "Finalisation du paiement…",
    activating: "Activation en cours",
    trialLabel: "Essai",
  },
  langs: { fr: "Français", en: "English", es: "Español", de: "Deutsch", it: "Italiano", pt: "Português", nl: "Nederlands", ru: "Русский" },
  stats: {
    title: "Statistiques", subtitle: "Vos abonnements par catégorie.",
    helper: "Tous les montants sont convertis vers votre devise principale.",
    breakdown: "Répartition", categoriesCount: "catégorie(s)",
    lockedTitle: "Pro requis", lockedDesc: "Passez Pro pour débloquer les statistiques détaillées.",
    tipTitle: "Bon à savoir", tipText: "Vous dépensez {{amount}} par an en abonnements ({{currency}}).",
  },
  categories: {
    video: "Vidéo & Streaming", music: "Musique", banking: "Banque & Finance",
    software: "Logiciels", gaming: "Jeux vidéo", fitness: "Sport & Fitness",
    cloud: "Stockage Cloud", news: "Presse & News", education: "Éducation",
    telecom: "Téléphonie & Internet", shopping: "Shopping & Livraison",
    insurance: "Assurance", services: "Services divers", other: "Autre",
  },
};
export default COMMON;
