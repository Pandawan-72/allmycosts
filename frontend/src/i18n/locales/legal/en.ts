const LEGAL_EN = {
  lastUpdated: "Last updated: July 15, 2026",
  editor: "All My Costs",
  contact: "dev@retro-spare.fr",
  country: "France",
  openMailto: "Contact us by email",
  aboutSection: "ABOUT",
  privacyTitle: "Privacy Policy",
  termsTitle: "Terms of Service",
  privacy: {
    intro: "This policy explains how All My Costs collects, uses and protects your data.",
    sections: [
      { title: "1. Publisher", body: "The All My Costs application is published by All My Costs. Contact: dev@retro-spare.fr." },
      { title: "2. Data collected", body: "Free: up to 10 expenses total (recurring and one-off combined), monthly/yearly total, base currency. Pro (one-time purchase, price shown on Google Play): unlimited expenses, statistics, PDF export, unlimited categories, AI-powered receipt scanning, receipt photo storage and sharing." },
      { title: "3. Local data", body: "All your recurring expenses, one-off expenses, receipt photos, and income are stored exclusively and locally on your device. They are never sent to our servers, except for the text described in article 4 below." },
      { title: "4. AI-powered receipt scanning (Pro feature)", body: "When you scan a receipt, the text is extracted directly on your device (offline, on-device text recognition). Only this text — never the photo itself — is sent to a third-party AI service (Anthropic) to extract the merchant name, amount, and date. The receipt photo remains stored only on your device." },
      { title: "5. Data use", body: "We use your data only to: manage your Pro access, respond to support requests, and structure scanned receipt text (Pro feature)." },
      { title: "6. Third-party services", body: "All My Costs uses: RevenueCat (Pro purchases), Google Play (payments), open.er-api.com (exchange rates), Anthropic (structuring scanned receipt text, Pro feature)." },
      { title: "7. Retention", body: "Local data (including receipt photos) deleted on uninstall. Text sent for receipt scanning is not retained beyond immediate processing." },
      { title: "8. Your rights (GDPR)", body: "Access, correction, deletion, portability, objection. Contact us at dev@retro-spare.fr." },
      { title: "9. Security", body: "All communications encrypted via HTTPS/TLS, including receipt scanning. Payments secured by Google Play." },
      { title: "10. Minors", body: "App not intended for users under 13." },
      { title: "11. Contact", body: "dev@retro-spare.fr" }
    ]
  },
  terms: {
    intro: "These Terms govern the use of the All My Costs application.",
    sections: [
      { title: "1. Acceptance", body: "By using the App, you accept these Terms." },
      { title: "2. Free and Pro version", body: "Free: up to 10 expenses total (recurring and one-off combined), monthly/yearly total, base currency. Pro (one-time purchase, price shown on Google Play): unlimited expenses, statistics, PDF export, unlimited categories, AI-powered receipt scanning, receipt photo storage and sharing." },
      { title: "3. 15-day free trial", body: "On first use, each user gets a 15-day free trial with full Pro access." },
      { title: "4. Pro one-time purchase", body: "Pro access is available as a one-time purchase at the price shown on Google Play. No subscription, no recurring fees." },
      { title: "5. Restore purchases", body: "Use the Restore purchases button in settings with the same Google Play account." },
      { title: "6. Refunds", body: "Refunds are handled by Google Play per their policy." },
      { title: "7. Acceptable use", body: "Local data (including receipt photos) deleted on uninstall. Text sent for receipt scanning is not retained beyond immediate processing." },
      { title: "8. Intellectual property", body: "The App and its content are the exclusive property of All My Costs." },
      { title: "9. Limitation of liability", body: "All communications encrypted via HTTPS/TLS, including receipt scanning. Payments secured by Google Play." },
      { title: "10. Contact", body: "dev@retro-spare.fr" }
    ]
  }
};
export default LEGAL_EN;
